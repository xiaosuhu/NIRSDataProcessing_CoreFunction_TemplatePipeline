classdef AR_IRLS_Diagnostics < nirs.modules.AbstractGLM
...
%     specified explicitly in the stimulus design with BoxCar basis functions
    ...
    methods
        ...
        function diagnostic = runThis( obj, data )
            vec = @(x) x(:);
            ...
            for i = 1:numel(data)
                ...
                if(rank([X C]) < size([X C],2) & obj.goforit)
                    ...
                    V=V(:,lst);
                    [stats,diagnostic{i}] = nirs.dataqualitycontrol.ar_irls_diagnostic( d, U(:,lst)*s(lst,lst), round(4*Fs) );
                    ...
                    % run regression
                    [stats,diagnostic{i}] = nirs.dataqualitycontrol.ar_irls_diagnostic( d, [X C], round(4*Fs) );
                end
                % put stats
                ncond = length(names);
