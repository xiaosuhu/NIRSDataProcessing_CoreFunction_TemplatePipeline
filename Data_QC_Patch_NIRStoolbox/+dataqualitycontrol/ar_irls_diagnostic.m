function [stats,S] = ar_irls_diagnostics( d,X,Pmax,tune )
...
%         Xfiltered=[];
    ...
    for i = 1:nChan
        ...
        while norm(B-B0)/norm(B0) > 1e-2 && iter < maxiter
            ...
            % perform IRLS
            [B, S{i}] = nirs.math.robustfit(Xf,yf,'bisquare',tune,'off');
       ...
       % stats.dfe = length(yf)-sum(U(:).*U(:));
        ...
        Xfall{i}=wXf;
        stats.covb(:,:,i) = 1.1265*L*S{i}.mad_s^2;
        stats.w(:,i) = S{i}.w;
        stats.a{i} = a;
        stats.sigma2(i)=1.1265*S{i}.mad_s^2;
        
        stats.tstat(:,i) = stats.beta(:,i)./sqrt(diag(S{i}.covb));
