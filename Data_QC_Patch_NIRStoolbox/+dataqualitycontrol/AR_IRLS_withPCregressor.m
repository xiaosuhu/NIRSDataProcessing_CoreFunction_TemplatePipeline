classdef AR_IRLS_withPCregressor < nirs.modules.AbstractGLM
    ...
    methods
        ...
        function S = runThis( obj, data )
            ...
            for i = 1:numel(data)
                ...
                % get a moving average component
                
                
                % get PC regressors
                ncomp=.3;
                % HbO PC
                mhbo = mean(d(:,1:2:end-1),1);
                dhbo = bsxfun(@minus, d(:,1:2:end-1), mhbo);
                [uhbo, shbomat, vhbo] = svd(dhbo,'econ');
                shbo = diag(shbomat);
                try
                    nhbo=max(find(cumsum(shbo)/sum(shbo)<=ncomp));
                catch
                    nhbo=1;
                end

                % The first nhbo PCs are defined as noises
                disp(['Using first ' num2str(nhbo) ' components (' num2str(sum(shbo(1:nhbo))/sum(shbo)*100) '%) as PC regressors of HbO  (Zscored)']);
                shbomat(nhbo+1:end,nhbo+1:end)=0;
                PChbo=uhbo(:,1:nhbo)*shbomat(1:nhbo,1:nhbo);
                PChboZ=zscore(PChbo);
                
                % HbR PC
               mhbr = mean(d(:,2:2:end),1);
                dhbr = bsxfun(@minus, d(:,2:2:end), mhbr);
                [uhbr, shbrmat, vhbr] = svd(dhbr,'econ');
                shbr = diag(shbrmat);
                try
                    nhbr=max(find(cumsum(shbr)/sum(shbr)<=ncomp));
                catch
                    nhbr=1;
                end

                % The first nhbr PCs are defined as noises
                disp(['Using first ' num2str(nhbr) ' components (' num2str(sum(shbr(1:nhbr))/sum(shbr)*100) '%) as PC regressors for HbR (Zscored)']);
                disp(' ');
                shbrmat(nhbr+1:end,nhbr+1:end)=0;
                PChbr=uhbr(:,1:nhbr)*shbrmat(1:nhbr,1:nhbr);
                PChbrZ=zscore(PChbr);
                ...
                % check model
                obj.checkRank( [X*10 C PChboZ PChbrZ] )
                obj.checkCondition( [X*10 C PChboZ PChbrZ] )
                ...
                if(rank([X C]) < size([X C],2) & obj.goforit)
                    ...
                    % run regression
                    stats = nirs.math.ar_irls( d,  [X*10 C PChboZ PChbrZ], round(4*Fs) );
                end
                % put stats
                ncond = length(names);
